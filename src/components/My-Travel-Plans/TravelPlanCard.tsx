  ...
  return (
    <div className="bg-white border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
      ...
      <div className="flex items-center justify-between gap-2 mt-6 pt-4 border-t border-gray-100">
        {/* FIXED LINK: Path changed to /my-travel-plans/ */}
        ...
        {isOwner && onEdit && onDelete && (
          <div className="flex gap-2">
            <Button variant="ghost" size="icon" onClick={() => onEdit(plan)} className="hover:text-amber-600">
              <Edit2 className="w-4 h-4" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => onDelete(plan)} className="hover:text-red-600">
